...
print("random seed is {:d}".format(seed))
...
class Net(torch.nn.Module):
    def __init__(self, num_features, hidden_dim, embed_dim, last_layer_complex=False):
        ...
        if last_layer_complex:
            self.last_layer = torch.nn.Linear(8*hidden_dim, embed_dim, dtype=torch.complex128)
        else:
            self.last_layer = torch.nn.Linear(8*hidden_dim, embed_dim)
...
def run_exp(embed_dim, embed_type, epochs):
    ...
    model = Net(num_features=num_features, hidden_dim=2048, embed_dim=embed_dim, last_layer_complex=('complex' in embed_type)).to(device)
